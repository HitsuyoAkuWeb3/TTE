        ...
        <div className="max-w-4xl mx-auto w-full animate-fade-in space-y-8 font-mono">
            <div className="border border-zinc-800 bg-black p-8 relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-zinc-800 text-zinc-400 px-3 py-1 text-[10px]">
                    PHASE 02: VALUE CHEMISTRY
                </div>

                <div className="mb-6">
                    <button onClick={onBack} className="text-xs text-zinc-500 hover:text-white transition-colors">&larr; RETURN TO PREVIOUS PHASE</button>
                </div>
                ...
                {status === 'IDLE' && (
                    <div className="flex flex-col items-center justify-center py-20 border border-zinc-900 border-dashed">
                        ...
                        <Button onClick={handleRadarScan}>INITIALIZE RADAR SCAN</Button>
                    </div>
                )}

                {status === 'RADAR_SCANNING' && (
                    <div className="py-20 text-center animate-pulse">
                        <p className="text-zinc-400">[ SCANNING NICHE FORUMS... ]</p>
                    ...
                    <div className="space-y-8 animate-fade-in">
                        <div className="grid md:grid-cols-2 gap-8">
                            <div className="p-4 border border-red-900/30 bg-red-950/5">
                                <h4 className="text-[#FF2A2A] text-xs font-bold mb-2 uppercase">THE FATAL WOUND</h4>
                                <p className="text-zinc-300 text-sm italic">"{radarData.fatalWound}"</p>
                            </div>
                            <div className="p-4 border border-zinc-800 bg-zinc-900/20">
                                <h4 className="text-zinc-500 text-xs font-bold mb-2 uppercase">THE SACRED COW</h4>
                                <p className="text-zinc-400 text-sm">"{radarData.sacredCow}"</p>
                            </div>
                        </div>

                        <div>
                            <h4 className="text-zinc-500 text-[10px] uppercase mb-3 tracking-widest">Shadow Belief Inventory</h4>
                        ...
                        <div className="pt-6 border-t border-zinc-900 flex justify-between items-center">
                            ...
                            <Button onClick={handleSynthesizeTov} variant="gold">ARCHITECT THEORY OF VALUE</Button>
                        </div>
                    </div>
                )}
                ...
                {status === 'DRAFTING_TOV' && (
                    <div className="py-20 text-center space-y-4">
                        <div className="animate-pulse">
                            <p className="text-[#eab308]">[ THE FORGE IS ACTIVE ]</p>
                        </div>
